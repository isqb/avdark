...
avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
        ...
        for (int i = 0; i < self->assoc; ++i) {
            ...
            if (self->lines[offset + i].valid && self->lines[offset + i].tag == tag) {
                hit = 1;
                self->lines[offset + i].access_count = 0;
            } else {
                // See if current entry is invalid. If it is,
                // make it potential candidate for replacement
                if (!self->lines[offset + i].valid) {
                    replace_index = i;
                    found_invalid = 1;
                } else {
                    // Aging
                    self->lines[offset + i].access_count++;

                    // If we don't have any invalid CLs for replacement,
                    // check if this entry is the oldest (LRU)
                    if (!hit && !found_invalid) {
                        if (self->lines[offset + i].access_count > max_age) {
                            max_age =  self->lines[offset + i].access_count;
                            replace_index = i;
                        }
                    }
                }
        ...
        // Do replacement on the oldest / invalid entry
        if (!hit) {
            self->lines[offset + replace_index].valid = 1;
            self->lines[offset + replace_index].tag = tag;
            self->lines[offset + replace_index].access_count = 0;
        }
